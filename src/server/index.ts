...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (request.method === "GET" && reviewMatch) {
      ...
      return Response.json({ ...row, contributors: JSON.parse(row.contributors || "[]") });
    }

    // GET /api/dashboard — aggregated stats for the dashboard
    if (request.method === "GET" && url.pathname === "/api/dashboard") {
      const [stats, primaryRepo, findingsByAgent, reviewsByDay, severitySplit, topIssues] =
        await Promise.all([
          env.DB.prepare(`
          ...
          `).all<{ title: string; agent: string | null; severity: string; cnt: number }>()
        ]);
      ...
      return Response.json({
        ...
        findingsByAgent: findingsByAgent.results.map((r) => ({ agent: r.agent, count: r.cnt })),
        reviewsByDay: reviewsByDay.results.map((r) => ({ day: r.day, count: r.cnt })),
        severitySplit: severitySplit.results.map((r) => ({ severity: r.severity, count: r.cnt })),
    ...
    if (request.method === "DELETE" && reviewMatch) {
      await env.DB.prepare(`DELETE FROM reviews WHERE id = ?`).bind(reviewMatch[1]).run();
      return new Response(null, { status: 204 });
    }

    // GET /api/reviews/:id/findings — findings for a specific review
    const findingsMatch = url.pathname.match(/^\/api\/reviews\/([^/]+)\/findings$/);
    if (request.method === "GET" && findingsMatch) {
      const rows = await env.DB.prepare(
        `SELECT * FROM findings WHERE review_id = ? ORDER BY severity`
      ).bind(findingsMatch[1]).all();
      return Response.json(rows.results);
    }
