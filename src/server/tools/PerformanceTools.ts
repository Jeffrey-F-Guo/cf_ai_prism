...
const HOT_NAME_RE = /\b(check|handle|process|request|middleware|validate|get|post|put|delete|fetch|call|invoke|respond|route|dispatch|serve|execute)\b/i;
const COLD_NAME_RE = /\b(evict|cleanup|clean|init|migrate|cron|purge|setup|seed|destroy|bootstrap|teardown|prune|vacuum|flush|reset|rebuild)\b/i;
const LOOP_RE = /\b(for\s*\(|for\s+(?:const|let|var)\s+\w|while\s*\()|\.(?:forEach|map|filter|reduce|flatMap)\s*\(/;
const BOUNDS_RE = /(maxKeys|maxSize|maxItems|capacity|limit|max|size)\s*[:=]\s*(\d+)/gi;
const BOUNDS_REF_RE = /\b(maxKeys|maxSize|maxItems|capacity|limit|max)\b/gi;
const DB_CALL_RE = /\b(fetch|query|findOne|findById|\.where|db\.|prisma\.|supabase\.|execute|runQuery)\b/i;
  ...
  execute: async ({ diff }: { diff: string }) => {
    ...
    for (const { content } of addedLines) {
      ...
      if (m) {
        ...
        if (name.length > 1 && !["if", "for", "while", "new", "return"].includes(name) && !seenNames.has(name)) {
          seenNames.add(name);
      ...
      if (/^\s*[/*]/.test(content)) {
        const kw = content.match(/\b(cron|scheduled|startup|initialization|one.time|maintenance)\b/i)?.[0];
        if (kw && !coldDocKeywords.includes(`"${kw}"`)) coldDocKeywords.push(`"${kw}"`);
      }
    }
    ...
    for (let i = 0; i < addedLines.length; i++) {
      ...
      const lookahead = addedLines.slice(i + 1, i + 12).map((l) => l.content).join(" ");
    ...
    const refNames = [...allContent.matchAll(BOUNDS_REF_RE)]
      ...
      .filter((n, i, arr) => arr.indexOf(n) === i && !bounds.some((b) => b.toLowerCase().startsWith(`  - ${n} =`)));
    for (const name of refNames) {
      bounds.push(`  - ${name} referenced as loop bound (value from config or parameter)`);
    }
    ...
    for (let i = 0; i < addedLines.length; i++) {
      ...
      if (/\bawait\b/.test(lookahead) && DB_CALL_RE.test(lookahead)) {
        n1Indicators.push(`  - Line +${lineNum}: loop contains await on DB/fetch call — verify this cannot be batched`);
      }
    }

    // ── Build output ──
    const contextLines: string[] = [];
    if (hotFunctions.length) contextLines.push(`  Hot-path functions (per-request): ${hotFunctions.join(", ")}`);
    if (coldFunctions.length) contextLines.push(`  Cold-path functions (maintenance/cron): ${coldFunctions.join(", ")}`);
    if (coldDocKeywords.length) contextLines.push(`  Docstring keywords: ${coldDocKeywords.join(", ")}`);
    if (!contextLines.length) contextLines.push("  No function names or comments indicate hot/cold path classification.");
...
export const makeAnalyzeMemoryPatternsTool = (apiKey: string, claudeApiKey?: string, modelPref?: "claude" | "deepseek") =>
  tool({
    description:
...
export const makeFindBlockingOperationsTool = (apiKey: string, claudeApiKey?: string, modelPref?: "claude" | "deepseek") =>
  tool({
    description:
    ...
    inputSchema: z.object({
      code: z.string().describe("The code section to analyze for blocking operations"),
    }),
    execute: async ({ code }: { code: string }) => {
      try {
        const llm = modelPref === "claude" && claudeApiKey
