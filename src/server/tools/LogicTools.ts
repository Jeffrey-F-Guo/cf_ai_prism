...
const SKIP_VARS = new Set(["console", "Math", "Object", "Array", "String", "JSON", "Promise", "Date", "Error", "window", "document"]);
  ...
  execute: async ({ diff }: { diff: string }) => {
    ...
    for (const { lineNum, content } of addedLines) {
      if (/\w+!\.\w+/.test(content) || /\w+!\[/.test(content)) {
        nullRisks.push(`  - Line +${lineNum}: non-null assertion (!) — verify value cannot be null/undefined at this point`);
      }
    }
    ...
    for (const { lineNum, content } of addedLines) {
      ...
      if (m) {
        nullRisks.push(`  - Line +${lineNum}: result of .${m[1]}() chained directly — may return undefined`);
      }
    }
    ...
    for (const v of optVars) {
      if (nonOptVars.has(v)) {
        nullRisks.push(`  - Variable \`${v}\` uses optional chaining (?.) in some places but not others — inconsistent null safety`);
      }
    }
    ...
    for (const { lineNum, content } of addedLines) {
      if (/\.forEach\s*\(\s*async/.test(content)) {
        asyncIssues.push(`  - Line +${lineNum}: async callback inside .forEach() — forEach does not await; use for...of instead`);
      }
    }
    ...
    for (const { lineNum, content } of addedLines) {
      for (const name of asyncFnNames) {
        ...
        const chained = content.includes(".then(") || content.includes(".catch(");
        const isReturn = /\breturn\b/.test(content);
        if (called && !awaited && !chained && !isReturn) {
          asyncIssues.push(`  - Line +${lineNum}: \`${name}()\` called without await — defined as async in this diff`);
        }
      }
    }
    ...
    for (const { lineNum, content } of addedLines) {
      if (/if\s*\(\s*true\s*\)/.test(content)) {
        unreachable.push(`  - Line +${lineNum}: always-true condition: if (true)`);
      }
      if (/if\s*\(\s*false\s*\)/.test(content)) {
        unreachable.push(`  - Line +${lineNum}: always-false condition: if (false) — body is unreachable`);
      }
      const taut = content.match(/if\s*\(\s*(\w+)\s*===?\s*\1\s*\)/);
      if (taut) {
        unreachable.push(`  - Line +${lineNum}: tautological condition: \`${taut[1]} === ${taut[1]}\``);
      }
    }

    // ── Edge case checklist from function signatures ──

    const sigRE = /(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:async\s*)?\()\s*([^)]{10,})\s*\)/g;
    for (const sig of [...allContent.matchAll(sigRE)].slice(0, 4)) {
      const params = sig[1];
      const checks: string[] = [];
      if (/:\s*(?:string|String)/.test(params)) checks.push("empty string, whitespace-only");
      if (/:\s*(?:number|Number)/.test(params)) checks.push("zero, negative, NaN, Infinity");
      if (/:\s*(?:\w+\s*\[\]|Array)/.test(params)) checks.push("empty array");
      if (/[?]|null|undefined/.test(params))     checks.push("null/undefined optional params");
      if (/Promise|async/.test(params))           checks.push("concurrent calls, rejection handling");
...
export const makeTraceDataFlowTool = (apiKey: string, claudeApiKey?: string, modelPref?: "claude" | "deepseek") =>
  tool({
    description:
...
export const makeDetectRaceConditionsTool = (apiKey: string, claudeApiKey?: string, modelPref?: "claude" | "deepseek") =>
  tool({
    description:
