...
const COMPLEXITY_RE =
  /\b(if|else\s+if|while|for|case|catch)\b|&&|\|\||\?\?|\?\.|(?<!\?)\?(?!\?)/g;
const FUNC_DECL_RE =
...
export const patternAnalyze = tool({
  ...
  execute: async ({ diff }: { diff: string }) => {
    ...
    for (const fn of functions) {
      ...
      if (score > 10) {
        complexFindings.push(
          `  - ${fn.name}(): complexity ${score} (threshold: 10)`
        );
    ...
    if (!complexFindings.length && functions.length > 5) {
      complexFindings.push(
        "  All changed functions within complexity threshold."
      );
    }
    ...
    for (const fn of functions) {
      if (fn.lines.length > 40) {
        lengthFindings.push(
          `  - ${fn.name}(): ${fn.lines.length} added lines (threshold: 40)`
        );
      }
    }
    ...
    for (const { content } of addedLines) {
      const m = content.match(
        /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s*)?\()\s*([^)]{20,})\s*\)/
      );
      if (m) {
        const name = m[1] || m[2];
        const paramCount = m[3]
          .split(",")
          .filter((p) => p.trim().length > 0).length;
        if (paramCount > 4) {
          paramFindings.push(
            `  - ${name}(): ${paramCount} params (threshold: 4)`
          );
        }
      }
    }
    ...
    const camelNames = [
      ...allContent.matchAll(/\b[a-z][a-zA-Z0-9]*[A-Z][a-zA-Z0-9]*\b/g)
    ].map((m) => m[0]);
    const snakeNames = [
      ...allContent.matchAll(/\b[a-z][a-z0-9]*_[a-z][a-z0-9_]*\b/g)
    ].map((m) => m[0]);
    ...
    for (const fn of functions) {
      ...
      if (/\bsave\b|\binsert\b|\bupdate\b|\bpersist\b|\bdb\./i.test(body))
        ops.push("persist");
      if (ops.length >= 3) {
    ...
    for (const { lineNum, content } of addedLines) {
      if (
        /\bswitch\s*\(\s*\w+\.(?:type|kind|action|event)\s*\)/.test(content)
      ) {
        solidFindings.push(
          `  - Line +${lineNum}: switch on .type/.kind/.action field`
        );
      }
    }
...
export const makeSearchSimilarPatternsTool = (
  ai: Ai,
  ...
  tool({
    ...
    inputSchema: z.object({
      codeChunk: z
        .string()
        .describe("The function body or code chunk to search for"),
      repo: z
        .string()
        .describe(
          "The repo in owner/repo format, extracted from the ContentsURL in the diff"
        )
    }),
    execute: async ({
      codeChunk,
      ...
      try {
        ...
        if (!vector)
          return "Embedding generation failed — cannot query Vectorize.";
...
export const makeCheckArchitecturalPatternsTool = (
  apiKey: string,
  claudeApiKey?: string,
  ...
  tool({
    ...
    inputSchema: z.object({
      code: z
        .string()
        .describe("The code section to analyze for architectural issues")
    }),
