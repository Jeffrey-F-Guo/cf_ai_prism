...
import {
  performanceAnalyze,
  makeAnalyzeMemoryPatternsTool,
  makeFindBlockingOperationsTool
} from "../tools/PerformanceTools";
...
export class PerformanceAgent extends Agent<Env> {
  async analyzeCode(
    ...
    const analyzeMemoryPatterns = makeAnalyzeMemoryPatternsTool(
      this.env.DEEPSEEK_API_KEY,
      this.env.CLAUDE_API_KEY,
      modelPref
    );
    const findBlockingOperations = makeFindBlockingOperationsTool(
      this.env.DEEPSEEK_API_KEY,
      this.env.CLAUDE_API_KEY,
      modelPref
    );
