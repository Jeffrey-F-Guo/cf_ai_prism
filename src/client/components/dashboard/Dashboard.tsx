...
export function Dashboard({
  ...
  const countMap = new Map(
    (data?.reviewsByDay ?? []).map((r) => [r.day, r.count])
  );
  const velocityPoints = last7.map((d) => countMap.get(d) ?? 0);
  const maxVelocity = Math.max(...velocityPoints, 1);

  const { segments: donutSegments, total: totalFindings } = buildDonut(
    data?.severitySplit ?? []
  );
    ...
    <main className="mt-20 min-h-screen bg-[#fbf9f6]">
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        <header className="mb-12 flex justify-between items-end">
          <div>
            <h1 className="font-headline text-5xl text-[#1b1c1a] mb-2 tracking-tight">
              Executive Dashboard
            </h1>
          ...
          <div className="hidden md:flex items-center gap-3 bg-[#f5f3f0] px-4 py-2 rounded-xl border border-[#c7c4d7]/20">
            ...
            <div>
              <p className="font-headline text-sm text-[#2a14b4] leading-tight">
                All Reviews
              </p>
              <p className="text-[8px] uppercase tracking-widest text-[#777586]">
                global
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#c7c4d7]/15 flex flex-col gap-1">
            <span className="text-[#464554] text-sm font-medium">
              Total Reviews
            </span>
        ...
        <div className="grid grid-cols-12 gap-8 mb-12">
          ...
          <div className="col-span-12 lg:col-span-8 bg-[#f5f3f0] p-8 rounded-2xl">
            <div className="flex justify-between items-center mb-10">
              <h2 className="font-headline text-2xl text-[#1b1c1a]">
                Review Velocity
              </h2>
            ...
            <div className="h-48 flex items-end justify-between gap-2 mb-4">
              {last7.map((d, i) => {
                ...
                const h =
                  maxVelocity === 0
                    ? 5
                    : Math.max((c / maxVelocity) * 100, c > 0 ? 8 : 5);
          ...
          <div className="col-span-12 lg:col-span-4 bg-white p-8 rounded-2xl border border-[#c7c4d7]/15 flex flex-col justify-between">
            <h2 className="font-headline text-2xl text-[#1b1c1a] mb-8">
            ...
            <div className="relative w-48 h-48 mx-auto mb-8">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-bold text-[#1b1c1a]">
                  {totalFindings > 999
                    ? `${(totalFindings / 1000).toFixed(1)}k`
                    : totalFindings}
                </span>
                <span className="text-[10px] text-[#777586] uppercase">
                  Total Items
                </span>
              </div>
            </div>
          ...
          <div className="col-span-12 lg:col-span-5 bg-[#f5f3f0] p-8 rounded-2xl">
            <h2 className="font-headline text-2xl text-[#1b1c1a] mb-6">
              Agent Contribution
            </h2>
            {/* Column headers */}
            <div className="grid grid-cols-[1fr_auto_auto] gap-x-6 text-[10px] uppercase tracking-widest text-[#777586] pb-3 border-b border-[#c7c4d7]/30 mb-1">
              ...
              <span className="text-right w-16 leading-tight">
                % of all
                <br />
                findings
              </span>
            ...
            <div>
              {Object.entries(AGENT_META).map(([key, meta]) => {
                const agentData = data?.findingsByAgent.find(
                  (f) => f.agent === key
                );
                const total =
                  data?.findingsByAgent.reduce((s, r) => s + r.count, 0) ?? 0;
                ...
                return (
                  <div
                    key={key}
                    className="grid grid-cols-[1fr_auto_auto] gap-x-6 items-center py-4 border-b border-[#c7c4d7]/20 last:border-0"
                  >
                    <div>
                      <p className="text-sm font-medium text-[#1b1c1a] mb-1.5">
                        {meta.label}
                      </p>
                      <div className="h-1.5 w-full bg-stone-200 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-[#2a14b4] rounded-full"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
          ...
          <div className="col-span-12 lg:col-span-7 bg-white p-8 rounded-2xl border border-[#c7c4d7]/15">
            ...
            {topIssues.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 text-center">
                <span className="material-symbols-outlined text-[#c7c4d7] text-4xl mb-2">
                  bug_report
                </span>
                <p className="text-sm text-[#464554]">No findings yet</p>
                <p className="text-xs text-[#777586] mt-1">
                  Complete a review to see recurring patterns
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {topIssues.map((issue, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex items-center justify-between p-4 bg-[#fbf9f6] rounded-xl"
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm ${rankColors[i] ?? "bg-[#efeeeb] text-[#464554]"}`}
                        >
        ...
        <section className="mt-16">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="font-headline text-3xl text-[#1b1c1a]">
                Recent Review History
              </h2>
              <p className="text-[#777586] text-sm mt-1">
                Click any row to load the full review.
              </p>
            </div>
            <div className="flex items-center gap-4">
              {repos.length > 1 && (
              ...
              {reviewHistory.length > 5 && (
                ...
                >
                  ...
                  <span className="material-symbols-outlined text-base no-underline">
                    arrow_forward
                  </span>
                </button>
              )}
