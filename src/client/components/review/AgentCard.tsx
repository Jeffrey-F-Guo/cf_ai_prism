...
> = {
  security: {
    ...
    icon: "verified_user",
    description:
      "Reviewing encryption protocols and dependency vulnerabilities."
  },
  performance: {
    iconBg: "bg-[#c0bdff]/20",
...
export function AgentCard({ agent }: { agent: Agent }) {
  ...
  const lastTask =
    activeTasks[activeTasks.length - 1] ?? agent.tasks[agent.tasks.length - 1];

  return (
    <div
      className={`bg-white rounded-[24px] p-6 shadow-sm border border-[#c7c4d7]/10 hover:shadow-md transition-shadow ${isQueued ? "opacity-60" : ""}`}
    >
      {/* Header */}
      ...
      {/* Title + description */}
      <h3 className="text-xl font-headline mb-2 text-[#1b1c1a]">
        {agent.title}
      </h3>
      <p className="text-sm text-[#464554] mb-6 leading-snug">
        {config.description}
      </p>
